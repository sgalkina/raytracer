...
MyScreen::MyScreen(int w, int h, int r_x, int r_y, Camera camera, double focus,
                   DoubleVector up) {
  width = w;
  heigth = h;
  resolution_x = r_x;
...
Scene::Scene(Camera c, MyScreen sc, std::vector<Sphere> shapes,
             DoubleVector amb, Light l) {
  camera = c;
  screen = sc;
  shapes_set = shapes;
  ambient = amb;
...
Color Scene::get_specular_color(Sphere shape, DoubleVector point) {
  ...
  return light.specular_color * light.specular_koef *
